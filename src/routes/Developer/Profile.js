...
export default class AdvancedProfile extends Component {
  ...
  componentDidMount() {
    ...
    this.props.dispatch({ type: 'developer/get', payload: { id: appId } });
    this.props.dispatch({ type: 'developer/listAbilityUnderApp', payload: { id: appId } });
  }
  ...
  changeStatus = (e) => {
    const status = e.key;
    const appId = this.props.match.params.id;
    this.props.dispatch({ type: 'developer/updateStatus', payload: { id: appId, status } });
  }
  ...
  toggleClientSecret = () => {
    if (this.state.clientSecretToggleText === '显示') {
      this.setState({
        ...
        clientSecret: this.props.developer.curApp.clientSecret,
      });
    } else {
      this.setState({
        clientSecretToggleText: '显示',
  ...
  render() {
    ...
    return (
      <PageHeaderLayout
        ...
        action={<span><Button onClick={() => this.props.dispatch(routerRedux.push({ pathname: `/user/developer-app-manage/${this.props.location.query.developerId}`, query: { developerName: this.props.location.query.developerName } }))}>返回应用管理</Button><Button onClick={() => this.props.dispatch(routerRedux.push('/user/developer-manage'))}>返回开发者管理</Button></span>}
        content={description}
      >
        ...
        <Card title="应用能力列表" style={{ marginBottom: 24 }} bordered={false}>
          <AbilityTable loading={developer.loading} data={developer.curAbilityUnderAppList} dispatch={this.props.dispatch} />
        </Card>
