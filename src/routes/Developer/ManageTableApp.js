...
export default class ManageTableApp extends PureComponent {
  // 翻页
  ...
  // 删除
  handleDelete = (id) => {
    const developerId = this.props.developer;
    Modal.confirm({
      title: '您确定要删除该应用吗?',
      content: '删除该应用后将无法恢复!',
      onOk: () => {
        this.props.dispatch({ type: 'developer/deleteApp', payload: { id, developerId } });
      },
    });
  }
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        render: record => (
          <div>
            <Link to={{ pathname: `/user/developer-app-manage/profile/${record.id}`, query: { developerId: this.props.developer, developerName: this.props.developerName } }}>管理</Link>
